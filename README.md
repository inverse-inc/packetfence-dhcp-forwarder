...
DHCP-forwarder is based on gopacket and depends upon WinPCAP to select the packets it requires through a BPF, which is really fast. Captured traffic is then forwarded to a configured host and port. 


A default BPF is generated by the configuration generator. That filter can be manually modified if required by the user.
